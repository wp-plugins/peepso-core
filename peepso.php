...
//		add_action('wp', array(&$this, 'init_callback'), 15);
        add_action('init', array(&$this, 'init_callback'), 50);
        add_action('set_current_user', array(&$this, 'set_user'));

        // activation hooks
        register_activation_hook(__FILE__, array(&$this, 'activate'));
//		$sFile = dirname(__FILE__) . DIRECTORY_SEPARATOR . 'install' . DIRECTORY_SEPARATOR . 'activate.php';
    ...
    {
        load_plugin_textdomain('peepso', FALSE,
...
//PeepSo::log('  req=' . var_export($_REQUEST, TRUE));
    ...
    public function check_query()
    {
        ...
        if (isset($_GET['logout'])) {
            ...
            wp_logout();
            wp_redirect(self::get_page('activity'));
        } else
...
//self::log("adding directory [{$dirname}] to path list");
    ...
    {
        if (get_option('peepso_do_activation_redirect', FALSE)) {
            delete_option('peepso_do_activation_redirect');
            wp_redirect('admin.php?page=peepso_config&tab=filesystem');
            exit();
        }
    }
...
//				$sRet = $aRoles[0];
    ...
    /*
     * Returne the PeepSo working directory, adjusted for MultiSite installs
...
//				// user_can() doesn't behave as expected in WP 3.8
//				$roles = array_values($user->roles);
//				$ban = in_array('peepso_ban', $roles);
//				$for_approval = in_array('peepso_register', $roles) || in_array('peepso_verified', $roles);
//			} else {
//				$ban = user_can($user, 'peepso_ban');
//				$for_approval = user_can($user, 'peepso_register') || user_can($user, 'peepso_verified');
//			}
