...
class PeepSoConfigSections extends PeepSoConfigSectionAbstract
{
	...
	public function register_config_groups()
	{
		...
		# @todo #257 $this->config_groups[] = $this->_get_opengraph_group();
...
//		$this->config_groups[] = $this->_get_advancedsearch_group();
		$this->config_groups[] = $this->_get_memberspage_group();
	...
	private function _get_opengraph_group()
	{
		$section = 'site_opengraph_';

		$enable = array(
			'name' => $section . 'enable',
			'label' => __('Enable Open Graph', 'peepso'),
			'type' => 'yesno_switch',
			'int' => TRUE,
			'field_wrapper_class' => 'controls col-sm-8',
			'field_label_class' => 'control-label col-sm-4',
		...
		$og_image = array(
			'name' => $section . 'image',
			'label' => __('Image', 'peepso'),
			'type' => 'yesno_switch',
			'int' => TRUE,
			'field_wrapper_class' => 'controls col-sm-8',
			'field_label_class' => 'control-label col-sm-4',
			'value' => intval(PeepSo::get_option($section . 'image'))
		);

		$og_description = array(
			'name' => $section . 'description',
			'label' => __('Description', 'peepso'),
			'type' => 'text',
			'int' => TRUE,
			'field_wrapper_class' => 'controls col-sm-8',
			'field_label_class' => 'control-label col-sm-4',
			'value' => intval(PeepSo::get_option($section . 'description'))
		);
		return (array(
			'name' => 'opengraph',
			'title' => __('Open Graph', 'peepso'),
			'description' => __('Open Graph Description', 'peepso'),
			'fields' => array(/*$enable,*/ $og_description, $og_image),
			'context' => 'right'
		));
	}
	...
	private function _get_memberspage_group()
	{
		$section = 'site_memberspage_';

		$default_sorting = array(
			'name' => $section . 'default_sorting',
			'label' => __('Default sorting', 'peepso'),
			'type' => 'select',
			'options' => array(
			...
			'descript' => __('Sort order of the Members Page if nothing is searched', 'peepso'),
			'field_wrapper_class' => 'controls col-sm-8',
			'field_label_class' => 'control-label col-sm-4',
			'value' => PeepSo::get_option($section . 'default_sorting'),
		);

		return (array(
			'name' => 'memberspage',
			'title' => __('Members Page', 'peepso'),
			'description' => __('Controls the behaviour of the Members Page', 'peepso'),
			'fields' => array($default_sorting),
			'context' => 'left',
		));
